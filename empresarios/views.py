...
def listar_empresas(request):
    ...
    if not request.user.is_authenticated:
        return redirect("/usuarios/logar")
    if request.method =='GET':
        empresa = Empresas.objects.filter(user=request.user)
        filtro = request.GET.get("empresa")
        
        if filtro:
            empresa = empresa.filter(nome__icontains=filtro)
            render(request,'listar_empresas.html',{"empresa":empresa})
...
def empresa(request,id):
    ...
    if request.method == 'GET':
        ...
        propostas_investimentos_enviada = PropostaInvestimento.objects.filter(empresa=empresa).filter(status='PE')
...
def add_doc(request, id):
    ...
    if empresa.user != request.user:
        return redirect(f"/empresarios/listar_empresas/")
    if extensao[1] != 'pdf':
        messages.add_message(request, constants.ERROR,"Envie apenas arquivos PDF's")
    ...
    messages.add_message(request, constants.SUCCESS,"Arquivo adicionado com sucesso")
