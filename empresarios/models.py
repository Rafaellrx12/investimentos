...
class Empresas(models.Model):
    ...
    def status(self):
        if date.today() > self.data_final_captacao:
            return mark_safe('<span class="badge rounded-pill bg-primary">Em capitação</span>')
        return mark_safe('<span class="badge rounded-pill bg-success">Capitação finalizada</span>')
